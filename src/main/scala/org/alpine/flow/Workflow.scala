...
sealed trait Workflow {
  ...
  def contains(n: Node[_ <: Result]): Boolean

  def predecessorsOf[C <: OpConf: Manifest, I <: Result: Manifest, O <: Result: Manifest](n: Node[O]): Option[Connection[C, I, O]]

  def foreach[U](f: Node[_ <: Result] => U): Unit

  def leaves: Set[Node[_ <: Result]]

  def foldLeft[U](zero: U)(combOp: (U, Node[_ <: Result]) => U): U

  def replace(inWorkflow: Node[_ <: Result], newNode: Node[_ <: Result]): Try[Workflow]
...
object Workflow {
  ...
  def create(
    ...
    val leafNodes =
      if (connections.size == 0) {
        ...
        val parents = connections.foldLeft(Map.empty[String, Node[_ <: Result]])(
    ...
    new Workflow {
      ...
      override def predecessorsOf[C <: OpConf: Manifest, I <: Result: Manifest, O <: Result: Manifest](n: Node[O]): Option[Connection[C, I, O]] =
        child2parents.get(mkKeyConnection[C, I, O](n))

      override def contains(n: Node[_ <: Result]): Boolean =
        nodeIDs.contains(n.id)

      override def foreach[U](f: Node[_ <: Result] => U): Unit =
        nodes.foreach(f)

      override val leaves: Set[Node[_ <: Result]] =
        leafNodes

      override def foldLeft[U](zero: U)(combOp: (U, Node[_ <: Result]) => U): U =
        nodes.foldLeft(zero)(combOp)

      override def replace(inWorkflow: Node[_ <: Result], newNode: Node[_ <: Result]): Try[Workflow] = Try({
        ...
        if (contains(inWorkflow) && Util.equalManifests(inWorkflow.nodeType, newNode.nodeType)) {
          ...
          predecessorsOf(inWorkflow) match {
            ...
            case Some(conn) =>
              conn.replaceDestination(newNode.asInstanceOf[OperatorNode[OpConf, Result, Result]]) match {
  ...
  @inline private def mkKeyConnection[C <: OpConf: Manifest, I <: Result: Manifest, O <: Result: Manifest](n: Node[O]): MapKey[Node[O], Connection[C, I, O]] =
    MapKey(n)
